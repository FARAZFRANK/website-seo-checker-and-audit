...
TASKS_DIR="D:/wamp64/www/dev/wp-content/plugins/frank-website-seo-checker-and-audit/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in D:/wamp64/www/dev/wp-content/plugins/frank-website-seo-checker-and-audit/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf D:/wamp64/www/dev/wp-content/plugins/frank-website-seo-checker-and-audit/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
